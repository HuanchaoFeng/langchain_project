...
#### tip: 起步阶段，因为这是根据工作时一个项目思路构思的系统，没有太多用户反馈和产品规划
...
## 1、完成文档收集、清洗与向量存储
**tip：** RAG这一块只做了简单的嵌入，还未根据业务要求做出优化
...
* 登录接口开发、注册接口开发
    ```python
    新增user表，对username使用唯一性约束防止重名
    ```
* 用户权限开发
    ```python
    新增用户权限表，将权限分为3级
    level_1：仅可以请求对话和自己账户管理
    level_2: 在level_1的基础上，增加请求知识库存储的权限
    level_3: 管理员权限
    ```
